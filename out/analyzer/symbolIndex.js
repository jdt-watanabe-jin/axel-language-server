...
function buildSymbolIndex(rootNode, uri, knownGuiClassNames = new Set()) {
    const declarations = [];
    const declarationNameKeys = new Set();
...
function collectDeclarations(node, uri, containerNames, declarations, declarationNameKeys, knownGuiClassNames) {
...
function declarationsFromNode(node, uri, containerNames, knownGuiClassNames) {
    ...
    if ((0, nodeKinds_1.isDeclarationNodeType)(node.type)) {
        return declaratorNames(node).map(({ nameNode, declaratorNode }) => {
            ...
            return declarationFromNode(node, nameNode, kind, uri, containerNames, detailForDeclarator(node, nameNode, declaratorNode, kind), containerNameForDeclarator(containerNames, declaratorNode));
        });
    }
    if (node.type === 'parameter_declaration') {
        return declaratorNames(node).map(({ nameNode, declaratorNode }) => declarationFromNode(node, nameNode, 'parameter', uri, containerNames, detailForDeclarator(node, nameNode, declaratorNode, 'parameter')));
    }
    if (node.type === 'gins_definition') {
        const typeNode = node.childForFieldName('type');
        const nameNode = node.childForFieldName('name') ?? declaratorNameFromNode(node);
        if (typeNode !== null && nameNode !== null && (0, guiClassKinds_1.isGuiPartTypeName)(typeNode.text, knownGuiClassNames)) {
            return [declarationFromNode(node, nameNode, 'variable', uri, containerNames, { detail: `${typeNode.text} ${nameNode.text}`, typeName: typeNode.text }, containerNames.at(-1))];
        }
    }
    if ((0, nodeKinds_1.isTypeSpecifierNodeType)(node.type) && node.childForFieldName('body') !== null) {
...
function recoveredTypeDeclarationFromMalformedBody(node, uri, containerNames) {
...
function isRecoverableTypeSpecifierNode(node) {
    return (0, nodeKinds_1.isTypeSpecifierNodeType)(node.type) && node.childForFieldName('body') === null;
}
function nextNamedSibling(node) {
...
function macroDeclarationFromNode(node, uri) {
...
function macroParameterLabels(parametersNode) {
    const labels = [];
...
function enumMemberDeclarationsFromNode(enumNode, enumName, uri) {
...
function enumMemberDeclarationFromNode(enumerator, enumName, uri) {
    ...
    return [declarationFromNode(enumerator, nameNode, 'enumMember', uri, enumName === undefined ? [] : [enumName], { detail }, enumName)];
}
function declaratorNames(node) {
    const names = [];
...
function declaratorNameNode(node) {
    if (node.type === 'qualified_declarator') {
        return node.childForFieldName('name');
    }
    const nameNode = isDeclaratorNameNode(node) ? node : (0, syntaxTree_1.getDeclaratorName)(node);
...
function isDeclaratorNameNode(node) {
    return [
        'identifier',
        'class_name',
        'operator_declarator',
...
function addDeclarationReferenceExclusions(node, declarations, exclusionKeys) {
...
function addPreprocFunctionParameterKeys(node, exclusionKeys) {
...
function addNonIndexedDeclarationReferenceExclusions(rootNode, exclusionKeys) {
    function visit(node) {
        if (node.type === 'parameter_declaration') {
        ...
        if (node.type === 'enumerator') {
            const nameNode = node.childForFieldName('name');
            if (nameNode !== null) {
                exclusionKeys.add(rangeKey((0, syntaxTree_1.nodeToAnalysisRange)(nameNode)));
            }
        }
        for (const child of node.namedChildren) {
            visit(child);
...
function addDeclaratorNameKeys(node, exclusionKeys) {
    for (const declarator of declaratorNodes(node)) {
...
function addIdentifierKeys(node, exclusionKeys) {
    if (node.type === 'identifier') {
        exclusionKeys.add(rangeKey((0, syntaxTree_1.nodeToAnalysisRange)(node)));
    }
    for (const child of node.namedChildren) {
        addIdentifierKeys(child, exclusionKeys);
    }
}
function addQualifiedDeclaratorIdentifierKeys(node, exclusionKeys) {
    if (node.type === 'qualified_declarator') {
...
function isContainerDeclarationNode(node) {
    return node.type === 'function_definition'
        || node.type === 'class_specifier'
        || node.type === 'struct_specifier'
        || node.type === 'union_specifier';
}
function nestedContainerNames(node, containerNames, nodeDeclarations) {
...
function declarationFromNode(node, nameNode, kind, uri, containerNames, declarationDetail = { detail: kind }, containerName = containerNames.at(-1)) {
    const selectionRange = (0, syntaxTree_1.nodeToAnalysisRange)(nameNode);
    return {
...
function baseNameFromTypeSpecifier(node) {
...
function containerNameForDeclarator(containerNames, declaratorNode) {
    return qualifiedDeclaratorScope(declaratorNode) ?? containerNames.at(-1);
}
function qualifiedDeclaratorScope(node) {
    if (node.type === 'qualified_declarator') {
...
function detailForDeclarator(declarationNode, nameNode, declaratorNode, kind) {
    if (kind === 'function') {
...
function signatureFromFunctionDeclarator(label, declaratorNode) {
...
function findFunctionParameterList(node) {
...
function parameterLabels(parameterList) {
    const labels = [];
...
function functionDetail(declarationNode, declaratorNode) {
...
function typeTextFromNode(typeNode) {
...
function declaratorText(declaratorNode, nameNode) {
...
function collectReferences(rootNode, uri, declarationNameKeys) {
    const references = [];
    ...
    function visit(node) {
        ...
        if (isReferenceNameNode(node)) {
            const range = (0, syntaxTree_1.nodeToAnalysisRange)(node);
...
function isReferenceNameNode(node) {
    return node.type === 'identifier'
        || node.type === 'class_name'
        || node.type === 'gtop_class'
        || node.type === 'gins_class';
}
function isTypeReferenceNameNode(node) {
...
function memberReferenceFromNode(node, uri, callTargetKeys) {
    ...
    const range = (0, syntaxTree_1.nodeToAnalysisRange)(currentMember);
    return {
...
function memberAccessChain(node) {
...
function qualifiedReceiverReferenceFromNode(node, uri) {
    ...
    return {
        name: receiver.text,
        uri,
        range: (0, syntaxTree_1.nodeToAnalysisRange)(receiver),
        typeReference: true
    };
}
function qualifiedReferenceFromNode(node, uri, callTargetKeys) {
    if (node.type !== 'qualified_identifier' || node.parent?.type === 'qualified_identifier') {
        return undefined;
    }
...
function qualifiedIdentifierSegments(node) {
    const segments = [];
...
function declarationId(uri, range, name) {
    return `${uri}#${range.start.line}:${range.start.character}:${name}`;
}
function rangeKey(range) {
    return `${range.start.line}:${range.start.character}:${range.end.line}:${range.end.character}`;
}
function declarationKind(type, declaratorNode) {
