...
suite('getHover', () => {
    ...
    test('resolves method calls through dot and arrow receivers', () => {
        ...
        const dotHover = (0, hover_1.getHover)({
            analysis,
            position: { line: 1, character: 44 },
            workspaceIndex: new workspaceIndex_1.WorkspaceIndex()
        });
        const arrowHover = (0, hover_1.getHover)({
            analysis,
    ...
    test('resolves chained widget member access through each field type', () => {
        ...
        const childHover = (0, hover_1.getHover)({
            analysis,
            position: { line: 3, character: 31 },
            workspaceIndex: new workspaceIndex_1.WorkspaceIndex()
        });
        const buttonHover = (0, hover_1.getHover)({
            analysis,
    ...
    test('resolves function and variable token hovers in the real groupbox sample', function () {
        ...
        const forcedIncludePath = path.normalize('D:/projects/work/TypeScript/axel-extension/include/_axel_intellisense_def.h');
        if (!fs.existsSync(samplePath) || !fs.existsSync(forcedIncludePath)) {
            this.skip();
        }
        const text = fs.readFileSync(samplePath, 'utf8');
        const index = new workspaceIndex_1.WorkspaceIndex({ forcedIncludeFiles: [forcedIncludePath] });
        const analysis = index.indexOpenDocument({
            uri: (0, url_1.pathToFileURL)(samplePath).toString(),
            version: 1,
            text
        });
        assert.deepStrictEqual(analysis.diagnostics, []);
    ...
    test('does not throw for a syntax-error document', () => {
        ...
        assert.doesNotThrow(() => (0, hover_1.getHover)({
            analysis,
            position: { line: 0, character: 5 },
            workspaceIndex: new workspaceIndex_1.WorkspaceIndex()
        }));
...
function assertHoverText(analysis, workspaceIndex, position, expectedPlainText) {
    const hover = (0, hover_1.getHover)({ analysis, position, workspaceIndex });
...
function positionOf(text, lineText, tokenText) {
...
//# sourceMappingURL=hoverAnalyzer.test.js.map