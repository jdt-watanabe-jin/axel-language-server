...
suite('buildGuiIndex', () => {
    ...
    test('classifies direct GUI class bases', () => {
        const index = (0, guiIndex_1.buildGuiIndex)(parser.parse([
        ...
        assert.deepStrictEqual(index.map((guiClass) => ({
            name: guiClass.name,
    ...
    test('normalizes external GUI event receiver paths', () => {
        ...
        assert.deepStrictEqual(index[0].methods.map((method) => ({
            name: method.name,
    ...
    test('uses known GUI class names to classify reusable part instances', () => {
        const index = (0, guiIndex_1.buildGuiIndex)(parser.parse('class MyDialog : public GCDialog { CustomWidget custom; };').rootNode, uri, new Set(['CustomWidget']));
    ...
    test('does not throw for malformed GUI classes', () => {
        const index = (0, guiIndex_1.buildGuiIndex)(parser.parse('class Broken : public GCDialog { GCVBoxLayout { GCText recovered; ').rootNode, uri);
...
//# sourceMappingURL=guiIndex.test.js.map