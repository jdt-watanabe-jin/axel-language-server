...
suite('getSignatureHelp', () => {
    test('returns the first parameter for an empty call argument list', () => {
        ...
        const help = (0, signatureHelp_1.getSignatureHelp)({
            analysis,
            text,
            position,
    ...
    test('resolves forced-include function signatures', () => {
        ...
        const analysis = index.indexOpenDocument({
            uri: (0, url_1.pathToFileURL)(path.join(tempDir, 'main.axl')).toString(),
            version: 1,
            text
        });
        const help = (0, signatureHelp_1.getSignatureHelp)({
...
function analyzeMarked(markedText) {
    ...
    const text = markedText.replace('|', '');
    return {
        analysis: new documentAnalyzer_1.DocumentAnalyzer().analyzeDocument({
            uri: 'file:///main.axl',
            version: 1,
            text
        }),
...
//# sourceMappingURL=signatureHelpAnalyzer.test.js.map