...
suite('buildSymbolIndex', () => {
    ...
    test('assigns a deterministic declaration ID to a function', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('void main() {}').rootNode, uri);
    ...
    test('uses declared type text as variable and parameter detail', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse([
            'class Widget {} item;',
            'int count;',
    ...
    test('uses signature text as function detail', () => {
        ...
        assert.deepStrictEqual(index.declarations.filter((declaration) => declaration.kind === 'function').map((declaration) => ({
            name: declaration.name,
            detail: declaration.detail,
    ...
    test('indexes each supported declaration kind', () => {
        ...
        assert.deepStrictEqual(index.declarations.map((declaration) => `${declaration.kind}:${declaration.name}`), [
            'macro:N',
            'macro:MAX',
            'typedef:Count',
            'variable:value',
    ...
    test('indexes anonymous enum members', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('enum { GC_BTN_OK = 0x01 };').rootNode, uri);
        assert.deepStrictEqual(index.declarations.map((declaration) => ({
            name: declaration.name,
            kind: declaration.kind,
            detail: declaration.detail,
    ...
    test('records class base names for inheritance-aware lookup', () => {
        ...
        assert.deepStrictEqual(index.declarations.filter((declaration) => declaration.kind === 'class').map((declaration) => ({
            name: declaration.name,
    ...
    test('excludes macro definitions and parameters from references', () => {
        ...
        assert.deepStrictEqual(index.references.filter((reference) => ['N', 'MAX', 'a', 'b'].includes(reference.name)).map((reference) => reference.name), ['MAX', 'N']);
    });
    ...
    test('uses the name field and excludes identifier parts of qualified declarations', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('void C::f() {}').rootNode, uri);
        assert.deepStrictEqual(index.declarations.map((declaration) => ({
            id: declaration.id,
            name: declaration.name,
    ...
    test('indexes named type specifiers as declarations only when they have bodies', () => {
        ...
        assert.deepStrictEqual(index.declarations.map((declaration) => `${declaration.kind}:${declaration.name}`), ['variable:value', 'variable:p', 'class:Declared']);
    ...
    test('indexes parameter declarations and excludes their names from references', () => {
        ...
        assert.deepStrictEqual(index.declarations.filter((declaration) => declaration.name === 'x').map((declaration) => ({
            kind: declaration.kind,
            detail: declaration.detail
    ...
    test('does not create declarations for anonymous types', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('struct { int member; };').rootNode, uri);
        assert.deepStrictEqual(index.declarations.map((declaration) => `${declaration.kind}:${declaration.name}`), ['variable:member']);
    });
    test('returns declarations recovered from malformed documents', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('void broken( { int recovered;').rootNode, uri);
        assert.deepStrictEqual(index.declarations.map((declaration) => declaration.name), ['broken', 'recovered']);
    });
    test('returns non-declaration identifiers as unresolved references', () => {
        const index = (0, symbolIndex_1.buildSymbolIndex)(parser.parse('void main() { int local; local = 1; }').rootNode, uri);
    ...
    test('adds named GUI parts as variable declarations owned by their GUI container', () => {
        ...
        assert.deepStrictEqual(index.declarations.filter((declaration) => ['group', 'input'].includes(declaration.name)).map((declaration) => ({
            name: declaration.name,
            kind: declaration.kind,
            detail: declaration.detail,
            containerName: declaration.containerName,
...
//# sourceMappingURL=symbolIndex.test.js.map