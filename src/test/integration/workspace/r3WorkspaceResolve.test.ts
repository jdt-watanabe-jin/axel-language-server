...
suite('R3 Workspace Symbol Resolve cache', () => {
  test('shrinks responses and resolves repeated selections without extracting unchanged files', async () => {
    const root = createTempDir(); const file = path.join(root, 'main.axl');
    fs.writeFileSync(file, Array.from({ length: 100 }, (_, i) => `int symbol${i};`).join('\n'));
    ...
    try {
      ...
      assert.strictEqual(lazy.length, 100); assert.strictEqual(extractions, 1);
      const eager: WorkspaceSymbol[] = [];
      for (const symbol of lazy) {
        const result = await resolve(symbol, CancellationToken.None) as WorkspaceSymbol;
        assert.ok('range' in result.location);
        const { data: _data, ...withoutData } = result; void _data;
        eager.push(withoutData);
      }
      assert.strictEqual(extractions, 1, 'selecting symbols must reuse the index parse');
      const lazyBytes = Buffer.byteLength(JSON.stringify(lazy)); const eagerBytes = Buffer.byteLength(JSON.stringify(eager));
      assert.ok(lazyBytes < eagerBytes * 0.9, `lazy ${lazyBytes}, eager ${eagerBytes}`);
      process.stdout.write(`    R3 payload: eager=${eagerBytes}, lazy=${lazyBytes}, reduction=${((1 - lazyBytes / eagerBytes) * 100).toFixed(1)}%; extractions=${extractions}\n`);
