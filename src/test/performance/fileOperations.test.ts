...
suite('File operation performance', function () {
  this.timeout(10000);
  test('yields while scanning and returns no partial edits on cancellation', async () => {
    const root = createTempDir();
    const uri = (name: string) => pathToFileURL(path.join(root, name)).toString();
    fs.writeFileSync(path.join(root, 'api.h'), '');
    for (let i = 0; i < 12; i++) { fs.writeFileSync(path.join(root, `${i}.axl`), '#include "api.h"\n'); }
    const logs: string[] = [];
    ...
    try {
      ...
      assert.strictEqual(edits?.documentChanges?.length, 12, logs.join('\n'));
