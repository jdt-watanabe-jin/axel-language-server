...
For installation-specific latency, set `AXEL_PERF_SAMPLE`, `AXEL_PERF_SETTINGS` (JSON initialization options), `AXEL_PERF_RENAME_SYMBOL` and optionally `AXEL_PERF_RENAME_COUNT`, then run `npm run test:external -- --grep 'external rename performance'`. Use a local symbol unique to that sample. The test reports cold outline and warm rename times over stdio without applying edits; the warm local rename budget is 500ms. Include roots are lookup paths, not scan targets. Only explicit `forcedIncludeRoots` are recursively enumerated.
