...
The whole-project index starts when subtype discovery or Go to Implementation first needs it, rather than at server startup. That first search may wait for indexing. Later searches reuse the index, and changes refresh it in the background. Preparing a hierarchy or showing supertypes resolves only the required source context.
