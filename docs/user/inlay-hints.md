...
The server requests `workspace/inlayHint/refresh` only when the client advertises `workspace.inlayHint.refreshSupport`. Configuration changes, watched-file changes, source edits and background indexing that changes the semantic index refresh dependent hints. Opening and closing an unchanged, already indexed dependency reuses its analysis when the include context permits; hovering its parameter hints then preserves existing hints and Code Lens results. Closing an edited buffer or detecting changed disk content still invalidates dependent results.

For clients negotiating `tooltip`, `label.tooltip` or `label.location` in inlay-hint resolve support, uniquely resolved calls can provide parameter descriptions and declaration navigation on demand. Ambiguous calls may still show their agreed parameter name, but do not acquire a guessed documentation target. Resolve preserves the initial label values, positions and padding. Clients without these capabilities retain the existing name-only hints. Source, dependency and settings changes invalidate retained resolve data; request fresh hints after `ContentModified`. See [resolve contracts](../developer/deferred-details.md).
